...
func GetMessage(c *gin.Context) {
	...
	if err != nil {
		...
		var listMessage []entities.MessageResponse
		//var listMessage entities.MessageResponse
		result := db.Table("message").Select("message.id, message.message_content, message.is_read, sender_receive.message_id, sender_receive.sender_id, sender_receive.receive_id").Joins("JOIN sender_receive ON message.id = sender_receive.message_id").Where("sender_receive.sender_id = ? AND sender_receive.receive_id = ?", json.SenderId, json.ReceiveId).Or("sender_receive.sender_id = ? AND sender_receive.receive_id = ?", json.ReceiveId, json.SenderId).Scan(&listMessage)
